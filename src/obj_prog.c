        ...
        int type )
{
    ...
    for ( prg = obj->pIndexData->oprogs; prg != NULL; prg = prg->next )
    {
        ...
        {
            return ( lua_obj_program( g_mud_LS, trigger, prg->vnum, prg->script->code, obj, obj2, ch1, ch2, NULL, type, prg->script->security)
    ...
    for ( prg = obj->pIndexData->oprogs; prg != NULL; prg = prg->next )
    {
        if ( prg->trig_type == type
            ...
            && ( strstr(cap_all(trigger), cap_all(prg->trig_phrase)) != NULL
                    ...
                    {
                        return (lua_obj_program( g_mud_LS, trigger, prg->vnum, prg->script->code, obj, NULL, ch1, NULL, NULL, type, prg->script->security)
                               && ( obj ? !obj->must_extract : TRUE )
                               && ( ch1 ? !ch1->must_extract : TRUE )
                               && ( ch2 ? !ch2->must_extract : TRUE ) );
                    }
    ...
    for ( obj = ch->in_room->contents; obj != NULL; obj = next_obj )
    {
        ...
        if ( HAS_OTRIG(obj, OTRIG_COMMAND) )
        {
            for ( prg = obj->pIndexData->oprogs; prg != NULL; prg = prg->next )
            {
                ...
                {
                    continu = lua_obj_program(g_mud_LS, cmd_table[cmd].name, prg->vnum, prg->script->code, obj, NULL, ch, NULL, argument, OTRIG_COMMAND, prg->script->security);
                    if ( !continu )
                        return FALSE;
                }
            }
    ...
    for ( obj = ch->in_room->contents; obj != NULL; obj = next_obj )
    {
        ...
        if ( HAS_OTRIG(obj, OTRIG_TRY) )
        {
            for ( prg = obj->pIndexData->oprogs; prg != NULL; prg = prg->next )
            {
                ...
                {
                    found = TRUE;
                    continu = lua_obj_program( g_mud_LS, trigger, prg->vnum, prg->script->code, obj, NULL, ch, NULL, NULL, OTRIG_TRY, prg->script->security);
