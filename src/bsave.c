...
MEMFILE* mem_save_char_obj( CHAR_DATA *ch )
{
   ...
   log_string("mem_save_char_obj: start");
    ...
    {
        PERF_PROF_ENTER( prgod_, "save godfile" );
        FILE *fp;
...
static void bwrite_char( CHAR_DATA *ch, DBUFFER *buf )
{
    ...
    */
    ...
    bprintf( buf, "Survivor %d %d\n", ch->pcdata->survivor.remort, ch->pcdata->survivor.ascent);
...
static void bread_char( CHAR_DATA *ch, RBUFFER *buf )
{
   ...
   log_string("bread_char: start");
    ...
    case 'M':
        ...
        KEY( "MobDeathsA",ch->pcdata->mob_deaths_ascent, bread_number( buf ) );
	...
	*/
    ...
    case 'S':
        ...
        if (!str_cmp(word, "Survivor" ) )
        {
            ...
            ch->pcdata->survivor.ascent = bread_number(buf);
            fMatch = TRUE;
            break;
        }
