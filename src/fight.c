...
bool one_hit ( CHAR_DATA *ch, CHAR_DATA *victim, int dt, bool secondary )
{
    ...
    /* behead */
    if ( dt != gsn_massive_swing || per_chance(50) )
    {
        ...
        if (victim!=ch || !(!IS_NPC(victim) && IS_SET(victim->act, PLR_WAR)))
        {
                    ...
                    && !IS_SET( victim->in_room->room_flags, ROOM_ARENA)) 
            {
                ...
                if ( IS_NPC(killer) || killer == victim )
                {
                    ...
                    victim->pcdata->mob_deaths_remort++;
                    victim->pcdata->mob_deaths_ascent++;
                }
                else
...
static void make_corpse( CHAR_DATA *victim, CHAR_DATA *killer, bool go_morgue)
{
    ...
    snprintf( buf, sizeof(buf), corpse->name, name );
    strlcat( buf, " fresh", sizeof(buf) );
