...
bool is_mob_in_spec( MOB_INDEX_DATA *mob, char *msg, size_t msgsz )
{
    ...
    {
	snprintf( msg, msgsz, "lvl=%d", mob->level );
	return FALSE;
    }
    ...
    {
        snprintf( msg, msgsz, "wealth=%d%%", mob->wealth_percent );
        return FALSE;
    }
    ...
    {
        snprintf( msg, msgsz, "hp=%d%%", mob->hitpoint_percent );
        return FALSE;
    }
    ...
    {
        snprintf( msg, msgsz, "damage=%d%%", mob->damage_percent );
        return FALSE;
    }
    ...
    {
        snprintf( msg, msgsz, "hitroll=%d%%", mob->hitroll_percent );
        return FALSE;
    }
...
bool is_obj_in_spec( OBJ_INDEX_DATA *obj, char *msg, size_t msgsz )
{
    ...
    if ( value > spec )
    {
	snprintf( msg, msgsz, "ops=%d/%d", value, spec );
	return FALSE;
    }
    ...
    for ( aff = obj->affected; aff != NULL; aff = aff->next )
    {
        if ( is_affect_cap_hard(aff->location) )
        {
            ...
            if ( value > spec )
            {
                snprintf( msg, msgsz, "%s = %d/%d", name_lookup(aff->location, apply_flags), value, spec );
                return FALSE;
            }
        }
    }
	...
	{
	    snprintf( msg, msgsz, "nr=%d/%d", obj->value[0], maxnr );
	    return FALSE;
	}
	if ( obj->value[1] > maxdam )
	{
	    snprintf( msg, msgsz, "dam=%d/%d", obj->value[1], maxdam );
	    return FALSE;
	}
    }
    ...
    if ( obj->item_type == ITEM_WEAPON )
    {
        ...
        if ( value > spec )
        {
            snprintf( msg, msgsz, "dam=%d/%d", value, spec );
            return FALSE;
        }
    }
	...
	if ( value > spec )
	{
	    snprintf( msg, msgsz, "ac=%d/%d", value, spec );
	    return FALSE;
	}
    }
...
bool has_special( MOB_INDEX_DATA *mob, char *spec_name, char *msg, size_t msgsz )
{
    ...
    {
        snprintf( msg, msgsz, "%s", mob_spec_name );
        return TRUE;
    }
...
bool has_affect( OBJ_INDEX_DATA *obj, int loc, char *msg, size_t msgsz )
{
	...
	if ( aff->location == loc )
	{
	    snprintf( msg, msgsz, "%d %s", aff->modifier,
