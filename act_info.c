...
void do_lore ( CHAR_DATA *ch, char *argument )
{
	...
	*/
        send_to_char( "Hmm... you cant seem to place it from any tales you've heard.\n\r", ch );
...
/* Pkill Grading system removed, killed pk */
	    
            /* ** Pkill status ** */
/*          if( IS_SET(ch->act, PLR_PERM_PKILL) )
 *          {
 *          sprintf( buf, "{D|{x   %s:  Grade {W<{x%s{W>{x (%d points from %d kills, %d deaths)",
 ...
 */
            ...
            /* Mob kills/deaths  */
            /* sprintf( buf, "{D|{x  Mobkills:  %d kills, %d deaths.",
                   ch->pcdata->mob_kills, ch->pcdata->mob_deaths );
               for ( ; strlen_color(buf) <= LENGTH; strcat( buf, " " ));
                   strcat( buf, "{D|{x\n\r" );
                   add_buf( output, buf ); */

            /* Changed to add behead count submitted by Vodur */
            /* Imped Maedhros 10/1/2011 */
		...
		for ( ; strlen_color(buf) <= LENGTH; strcat( buf, " " ));
            ...
            /* This else statement shouldn't exist. It previously excluded
               score items from PKill players. Fixed. Astark - Oct 2012 */
            else
            {
            }
            /* End Astark October formatting updates */
...
void do_attributes( CHAR_DATA *ch, char *argument )
{
   ...
   "{D|{x {bInt:{x %3d(%3d)  {bWis:{x %3d(%3d)  {bDis:{x %3d(%3d)  {bCha:{x %3d(%3d)  {bLuc:{x %3d(%3d) {D|{x\n\r",
    ...
    if( IS_NPC(ch) || ch->level >= 25 )
    {
        ...
        /* ** Hitroll, damroll ** */
        sprintf( buf, "{D|{x {CHit{croll:{x %-5d   {CDam{croll:{x %-5d   {CSaves:{x %d",
...
/*void do_helper( CHAR_DATA *ch, char *argument )
{
	...
	if( IS_ACTIVE_HELPER(ch) )
	{
	    send_to_char( "Helper status set to 'inactive'.  Type 'helper' again to toggle.\n\r", ch );
...
void do_eqdb(CHAR_DATA *ch, char *argument)
{
//    OBJ_DATA *obj;
    char buf[MAX_STRING_LENGTH];
    AFFECT_DATA *paf;
...
*/
        ...
        {
	    nMatch++;
    fprintf( fp, "\n\r\n\rVNUM\n\r%d\n\rSDESC\n\r%s\n\r",obj->vnum, remove_color(obj->short_descr));
    fprintf( fp,
             "Object\n\r%s\n\rType\n\r%s\n\rExtra\n\r%s\n\rWeight\n\r%d\n\rLevel\n\r%d\n\r",
             obj->name,
             item_name( obj->item_type ),
...
//    if ( CAN_WEAR(obj, ITEM_TRANSLUCENT) )
    {
        fprintf(fp,"%s", CAN_WEAR(obj, ITEM_TRANSLUCENT)?"TRANS\n\rYES\n\r":"TRANS: NO\n\r" );
//	fprintf( fp, "It's translucent, allowing tattoos to shine through.\n\r" );
    }

  /*  if (obj->owner != NULL)
        fprintf(fp, "Owned by: %s\n\r", capitalize(obj->owner));*/
    ...
    case ITEM_WAND:
    case ITEM_STAFF:
        fprintf(fp, "Has %d charges of level %d",
            obj->value[2], obj->value[0] );
        //send_to_char( buf, ch );
    ...
    case ITEM_ARMOR:
        fprintf(fp,
                 "Armorclass\n\r%d pierce, %d bash, %d slash, and %d vs. magic.\n\r",
                 obj->value[0], obj->value[1], obj->value[2], obj->value[3] );
        //send_to_char( buf, ch );
        break;
    ...
    case ITEM_LIGHT:
        if ( obj->value[2] >= 0 )
            fprintf(fp, "It has %d hours of light remaining.\n\r", obj->value[2] );
        else
            fprintf(fp, "It is an infinite light source.\n\r" );
        //send_to_char( buf, ch );
        break;

    case ITEM_ARROWS:
        fprintf(fp, "It contains %d arrows.\n\r", obj->value[0] );
        //send_to_char( buf, ch );
        if ( obj->value[1] > 0 )
        {
            fprintf(fp, "Each arrow deals %d extra %s damage.\n\r",
                     obj->value[1], flag_bit_name(damage_type, obj->value[2]) );
            //send_to_char( buf, ch );
        }
        break;
...
/*    for ( paf = obj->affected; paf != NULL; paf = paf->next )
        if ( paf->detect_level >= 0 && paf->detect_level <= level )
            print_affect(ch, paf, FALSE);*/

    for ( paf = obj->affected; paf != NULL; paf = paf->next )
        //if ( paf->detect_level >= 0 && paf->detect_level <= level )
            print_affect(ch, paf, fp);
	fprintf(fp, "END\n\r");
