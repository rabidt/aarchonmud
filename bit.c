...
bool is_stat( const struct flag_type *flag_table )
{
    ...
    bug( "is_stat: flag_table not found", 0 );
    return FALSE;
}
...
#ifdef FLAG_DEBUG
    log_string( "flag_value: start" );
#endif

    /* Bobble: return just one value */
...
char *flag_string( const struct flag_type *flag_table, tflag bits )
{
    ...
    if ( is_stat(flag_table) )
    {
	bug( "flag_string: flag_table is stat not flag", 0 );
	return buf[cnt];
    }
...
char *i_flag_string( const struct flag_type *flag_table, long bits )
{
    ...
    for (flag = 0; flag_table[flag].name != NULL; flag++)
    {
	if ( I_IS_SET(bits, flag_table[flag].bit) )
	{
	    strcat( buf[cnt], " " );
	    strcat( buf[cnt], flag_table[flag].name );
	}
